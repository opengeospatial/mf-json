...
DEFAULT:: If an object member has a *default* value defined in this specification, the member has a default value and its pair (member/value) can be omitted in a MF-JSON document.
OPTIONAL:: If an object member is *optional*, its value allows a JSON `null` value.
