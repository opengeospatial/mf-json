...
Moving Features JSON Encodings (MF-JSON) has two encoding formats: *MF-JSON Trajectory* for instances of the MF_TemporalGeometry type with linear interpoation and
*MF-JSON Prism* that contains new members to describe instances of the MF_PrismGeometry (or MF_RigidTemporalGeometry) type in the ISO coneptual model.
