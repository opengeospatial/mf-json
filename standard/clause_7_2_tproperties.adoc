...
.Example {counter:example-num}: Examples of a ParametricValues object
[source, javascript]
{
...
is any string defined by an application as a dynamic attribure.
...
* `"interpolation"`: The `"interpolation"` member is default and has a JSON string or a URL to descirbe an interpolation method.
...
|=========
| Names | Descriptions
| Discrete| The sampling of the attribute occurs such that it is not possible to regard the series as continuous; Thus, there is no interplated value if latexmath:[t] is not an element in `"datetimes"`.
