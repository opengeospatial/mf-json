...
If no CRS instance can be so acquired in a file, the TemporalGeometry object assumes the default CRSs.
...
into the fixed local coordinate reference system whose bound is *-0.5 to 0.5* for each axis concerning the right-handed local engineering coordinate reference syste
and unit is *meter*. The origin of the local coordinate reference system is 0 for each axis is transformed
...
from the right-handed local engineering coordinate reference system to the Universal Transverse Mercator (UTM) system as a type of plane rectangular coordinate system.
...
[#taggregate, reftext='7.2.1.2']
===== 7.2.1.2. TemporalAggregateGeometry Object
