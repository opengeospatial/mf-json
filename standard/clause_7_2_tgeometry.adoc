...
If no CRS instance can be so acquired in a file, the default CRSs shall be applied to the object.

[#tgeometryjson, reftext='{example-caption} {counter:example-num}']
.Example {counter:example-num}: Common members of a TemporalGeometry object
[source, javascript]
{
  "type": "...", //(MANDATORY)
  "crs" : {...}, //(DEFAULT)
...
named `"datetimes"`, `"coordinates"`, `"interpolation"`, `"orientations"`, and `"base"` as shown in <<tprimitivejson>>, as well as the common members in <<tgeometryjson>>.
...
.Example {counter:example-num}: Members of a TemporalPrimitiveGeometry Object
[source, javascript]
{
...
| MovingPointCloud | The type represents the prism of a time-parametered point aggregate
...
[NOTE]
==========
When the `"trs"` member has the default temporal CRS (i.e., ISO8601) object, an instant is generally encoded as a a JSON string
of https://www.iso.org/iso-8601-date-and-time-format.html[ISO 8601] field-based formats using Z (e.g., `"2016-09-11T06:10:32Z"`) or
incremental time as the number of milliseconds since midnight (00:00 a.m.) on January 1, 1970 in UTC (e.g., `1465621816590`),
but others can be used such as:
-----
   YYYY (e.g., 1997)
   Year and month:
      YYYY-MM (e.g., 1997-07)
...
of leaf geometries of a temporal geometry. It does NOT allow a JSON `null` value nor an empty array.
...
[NOTE]
==========
* A single position is represented by a JSON array of numbers, which must contain two or more elements.
...
* A Polygon coordinate array is expressed as a JSON array of LinearRing coordinate arrays. A LinearRing is closed LineString with 4 or more single positions, allowing the same positions.
The first and last positions are equivalent (they represent equivalent points). For Polygons with multiple rings, the first must be the exterior ring and any others must be interior rings or holes.
...
* `"base"`: The `"base"` member is optional and it value is a URL to address an OBJ<<OBJ>>
...
into the fixed local coordinate reference system whose bound is *-0.5 to 0.5* for each axis and unit is *meter*.
...
from the local engineering coordinate reference system to the Universal Transverse Mercartor (UTM) system as a type of plane rectangular coordinate system.
...
To convert Euler angles to quaternion form, defining the angle around the X-axis (roll) as latexmath:[\psi], the Y-axix (pitch) as latexmath:[\theta], and the Z-axis (yaw) as latexmath:[\phi].
...
.The transformation of a 3D model by using the orientation matrices (Need to fix).
image::mf-orientation.png[mf-orientation, pdfwidth=65%, width=65%, align="center"]
...
A TemporalAggregateGeometry object represents a set of TemporalPrimitiveGeometry objects.
When a TemporalGeometry object has a `"type"` member with one of the value
of `"MultiMovingPoint"`, `"MultiMovingLineString"`, `"MultiMovingPolygon"`, and `"MovingGeometryCollection"`,
the object is specialized as a TemporalAggregateGeometry object with one additional *mandatory* member
named `"prisms"`. The value of the `"prisms"` member is represented by
a JSON array of a set of TemporalPrimitiveGeometry instances, having at least one element in the array.
...
* *MultiMovingPoint*: The type represents a trajectory of a set of time-parametered 0-dimensional (0D)
geometric primitive (Point), representing a multi-position at a time position (instant) within its temporal domain.
The leaf geometry at a time position is the union of each leaf of moving point members at the same time.
* *MultiMovingLineString*: The type represents the prism of a set of time-parametered 1-dimensional (1D)
geometric primitive (LineString), whose leaf geometry at a time position is the union of each leaf of moving linestring members at the same time.
* *MultiMovingPolygon*: The type represents the prism of a set of time-parametered 2-dimensional (2D)
geometric primitive (Polygon), whose leaf geometry at a time position is the union of each leaf of moving polygon members at the same time.
