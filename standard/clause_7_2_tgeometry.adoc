...
it is interpreted by `"Linear"` as the default value. The motion of the geometry of a moving feature requires both translational and rotational motion,
MF-JSON Prism recommends using the `"base"` and `"orientations"` member with the `"coordinates"` that represents a MovingPoint for avoiding ambiguities of the object boundary.
...
each axis concerning the right-handed local engineering coordinate reference system (i.e., right-handed orthogonal axis system, latexmath:[Z=X\times Y]) and unit is *meter*.
...
The origin of the local coordinate reference system is 0 for each axis and is translated into the centroid of each leaf geometry of a temporal geometry.
