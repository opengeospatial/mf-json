...
image::mf-overview.png[Overview, pdfwidth=100%, width=60%]
...
The movement of a moving feature is represented as a JSON object where the type members value shall be one of the following strings: "_MovingPoint_", "_MovingLineString_", "_MovingPolygon_", "_MultiMovingPoint_", "_MultiMovingLineString_", "_MultiMovingPolygon_", or "_MovingGeometryCollection_".
...
[width="99%", cols="1,^4,4", options="header"]
|=========
| Types | Descriptions | _Comments_
| Discrete | image:discrete.png[Discrete, width=60%] | _There is no interpolation position between two successive positions._
| Stepwise | image:stepwise.png[Stepwise, width=60%] | _The interpolation position between two successive positions equals to the first position._
| Linear   | image:linear.png[Linear, width=60%] | _The new position is found from the linear interpolation formula with the two successive positions. Default_
|=========
...
.[Interpolation Formulas]
A temporal geometry may have a several interpolation formulas within its temporal domain. An element of interpolation formulas is represented by field `"coefficients"` and `"period"`. The new position at a given time position is derived from a `"coefficients"` instance as a multidimensional array of *polynomial interpolation formulas* of latexmath:[(x, y, z)] coordinates during a particular time period. If the time position does not belong to any time period of the elements of formula, there is no interpolation position at that time. The order of each formula array SHALL follow latexmath:[x] (longitude), latexmath:[y] (latitude), latexmath:[z] (altitude) order. The number of elements in each formula during a particular time period is the same. The time periods between any two elements of formulas only allows empty or 0-dimensional intersection.
